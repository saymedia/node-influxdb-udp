...
describe('InfluxDbUdp', function() {
  ...
  describe('#send()', function () {
    it('should format a simple message properly', function () {
      client.send('foo', 1);
      assert.notEqual(client.udpMessage.indexOf('foo value=1i'), -1);
    });
    ...
    it('should format a complex message properly', function () {
      ...
      assert.notEqual(client.udpMessage.indexOf(
          "foo,tag0=two,tag1=one field1=1i,field2=3i,field3=2i"), -1);
    });
    ...
    it('should escape spaces in tag values', function () {
      ...
      assert.notEqual(client.udpMessage.indexOf("foo,tag=test\\ test value=1i"), -1);
    });
    ...
    it('should represent integers properly for field values', function () {
      ...
      assert.notEqual(client.udpMessage.indexOf("foo value=12345i"), -1);
    });
  });
  ...
  describe('#sendPoints()', function () {
    ...
    it('should apply tags to each point', function () {
      ...
      assert.notEqual(client.udpMessage.indexOf("foo,tag=x value=1i"), -1);
      assert.notEqual(client.udpMessage.indexOf("\nfoo,tag=x value=2i"), -1);
      assert.notEqual(client.udpMessage.indexOf("\nfoo,tag=x value=3i"), -1);
    });

    it('should support tag at the point and global level', function () {
      client.sendPoints("foo", [[1, {"tag1": "x"}], 2, 3], {"tag2": "x"});
      assert.notEqual(client.udpMessage.indexOf("foo,tag1=x,tag2=x value=1i"), -1);
      assert.notEqual(client.udpMessage.indexOf("\nfoo,tag2=x value=2i"), -1);
      assert.notEqual(client.udpMessage.indexOf("\nfoo,tag2=x value=3i"), -1);
    });
  });
});
